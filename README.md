...
El CLI también acepta `--include`, `--exclude`, `--max-files` y `--max-file-size-bytes` para acotar la ingesta. Cuando se requiere análisis por contenido, `--parse-source-text` habilita una etapa explícita y controlada que lee únicamente archivos candidatos TypeScript/TSX dentro del límite `--max-source-text-bytes`, alimenta `parser-typescript` con `sourceText` y adjunta el resultado de `analyzer-static` al export JSON. Esta etapa no ejecuta scripts ni código del repositorio analizado, no interpreta `package.json`, no resuelve imports contra filesystem y todavía no crea nodos de símbolos ni edges `imports`/`exports` en el grafo.
