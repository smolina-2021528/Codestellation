...
El parser y el analizador estático todavía no calculan hashes, no crean un `Program`, no usan type checker, no resuelven módulos, no analizan referencias profundas ni interpretan manifests. La lectura de texto fuente existe únicamente en el CLI de forma opt-in y bajo límites explícitos.
...
El CLI también acepta `--include`, `--exclude`, `--max-files` y `--max-file-size-bytes` para acotar la ingesta. Cuando se requiere análisis por contenido, `--parse-source-text` habilita una etapa explícita y controlada que lee únicamente archivos candidatos TypeScript/TSX dentro del límite `--max-source-text-bytes`, alimenta `parser-typescript` con `sourceText`, adjunta el resultado de `analyzer-static` al export JSON, genera edges `imports`/`exports` iniciales cuando hay targets canónicos seguros y crea nodos `symbol` con edges `declares` desde los resultados del parser. Esta etapa no ejecuta scripts ni código del repositorio analizado, no interpreta `package.json`, no resuelve imports relativos contra filesystem y todavía no crea edges `references` ni `calls`.
