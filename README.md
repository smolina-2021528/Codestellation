...
El parser y el analizador estático todavía no leen archivos desde disco, no calculan hashes, no crean un `Program`, no usan type checker, no resuelven módulos, no analizan referencias profundas, no integran todavía resultados del scanner y no construyen nodos ni relaciones del grafo. El siguiente límite recomendado es iniciar `@codestellation/graph-builder` para crear nodos de proyecto, carpetas y archivos desde los contratos ya disponibles.
